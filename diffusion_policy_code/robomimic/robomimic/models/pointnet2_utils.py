...
class PointNet2EncoderSample(nn.Module):
    def __init__(self, in_channels=3, out_channels=256, use_bn=False):
        ...
        # pn2.1_r_0.04
        self.sa1 = PointNetSetAbstraction(npoint=512, radius=0.02, nsample=32, in_channel=in_channels, mlp=[64, 64, 128], group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=128, radius=0.045, nsample=64, in_channel=128 + 3 + 1, mlp=[128, 128, 256], group_all=False)
        self.sa3 = PointNetSetAbstraction(npoint=None, radius=None, nsample=None, in_channel=256 + 3 + 1, mlp=[256, 512, 1024], group_all=True)
        
        # two layer version
        # self.sa1 = PointNetSetAbstraction(npoint=512, radius=0.025, nsample=18, in_channel=in_channels, mlp=[64, 64, 256], group_all=False)
        # self.sa2 = PointNetSetAbstraction(npoint=None, radius=None, nsample=None, in_channel=256 + 3, mlp=[256, 512, 1024], group_all=True)
...
class PointNet2Encoder(nn.Module):
    ...
    def forward(self, xyz):
        ...
        assert D == self.in_channels
        if D > 3:
            init_attn = torch.mean(torch.zeros_like(xyz[:, 3:, :]),dim=1, keepdim=True)
            norm = xyz[:, 3:, :]
            xyz = xyz[:, :3, :]
            # norm = 1 -norm
        else:
            norm = None
        l1_xyz, l1_points, l1_attention = self.sa1(xyz, norm, init_attn)
        l1_points = torch.cat([l1_points, l1_attention], dim = 1)
        l2_xyz, l2_points, l2_attention = self.sa2(l1_xyz, l1_points, l1_attention)
        l2_points = torch.cat([l2_points, l2_attention], dim=1)
        l3_xyz, l3_points, l3_attention = self.sa3(l2_xyz, l2_points, l2_attention)
