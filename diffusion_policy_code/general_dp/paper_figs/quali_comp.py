...
def draw_for_one_ckpt_name(ckpt_name, ckpt_map_dict, s_idx, e_idx, view_ctrl_info, dataset_dir, epi_path, only_action=False):
    ...
    out_dir = f"{dataset_dir}/quali_comp_{ckpt_name}"
    ...
    # cv2.imwrite(f'{out_dir}/pca_example.png', example_pca_norm)
    # cv2.imwrite(f'{out_dir}/example.png', example_img)
    ...
    for i in range(s_idx, e_idx):
        ...
        if 'd3fields' in vis_shape_meta['obs']:
            ...
            vis_shape_meta['obs']['d3fields']['info']['boundaries']['x_lower'] = -0.2
        ...
        if ckpt_name == 'our':
            ...
            o3d_vis.update_pcd(d3fields_o3d, mesh_name='input_d3fields')
        ...
        if only_action:
            ...
            o3d_vis.render(action_names_ls, save_name=f'3d_action_{i}')
            if ckpt_name == 'our':
                o3d_vis.render(['input_d3fields'], save_name=f'input_{i}')
        ...
        for view_idx in range(4):
            ...
            in_bound_mask = (pred_act_pos_in_pix[:, 0] >= 0) & (pred_act_pos_in_pix[:, 0] < img.shape[1]) \
                        & (pred_act_pos_in_pix[:, 1] >= 0) & (pred_act_pos_in_pix[:, 1] < img.shape[0])
            ...
            cv2.imwrite(f'{out_dir}/action_in_view_{view_idx}_{i}.png', img)
...
# }
perspective_view_ctrl_info = {
    "front" : [ 0.55361720334703601, 0.6269773061292675, 0.54809438033708546 ],
    "lookat" : [ 0.40675679682081112, -0.0759232760293303, 0.076474288008170022 ],
    "up" : [ -0.40965112278391286, -0.36798762862251977, 0.83472813704981452 ],
    "zoom" : 0.76000000000000001
}
perspective_view_ctrl_info = {
    "front" : [ -0.00047144305498102201, 0.99534156781004834, -0.09641027502751498 ],
    "lookat" : [ 0.39881250551416231, -0.074211982139727087, 0.11828207804118071 ],
    "up" : [ -0.023977480464595521, 0.096371316310741972, 0.99505660634111914 ],
...
data_root = "/media/yixuan_2T/diffusion_policy"
# dataset_dir = f"{data_root}/data/real_aloha_demo/quali_2_v4/dir_1"
# dataset_dir = f"{data_root}/data/real_aloha_demo/quali_2_v4/dir_2"
dataset_dir = f"{data_root}/data/real_aloha_demo/quali_2_v4/novel_2"
...
#     'dp_rgbd': '/media/yixuan_2T/diffusion_policy/data/outputs/open_pen/2024.01.28_pen_Guang_open_pen_original_rgbd/checkpoints/latest.ckpt',
# }
# data_root = "/media/yixuan_2T/diffusion_policy"
# dataset_dir = f"{data_root}/data/real_aloha_demo/quali_comp_qopen_pen/dir_0"
# epi_path = f"{dataset_dir}/episode_0.hdf5"

for ckpt_name in ["our", "raw_pcd", "dp", "dp_rgbd"]:
# for ckpt_name in ["dp_rgbd"]:
    draw_for_one_ckpt_name(ckpt_name, ckpt_map_dict, s_idx=0, e_idx=20, view_ctrl_info=perspective_view_ctrl_info, dataset_dir=dataset_dir, epi_path=epi_path, only_action=False)
    # draw_for_one_ckpt_name(ckpt_name, ckpt_map_dict, s_idx=20, e_idx=30, view_ctrl_info=front_view_ctrl_info, dataset_dir=dataset_dir, epi_path=epi_path, only_action=True)
