...
from d3fields.utils.draw_utils import np2o3d, aggr_point_cloud_from_data, o3dVisualizer, gen_circular_view_ctrl_info
...
output_dir = f"{dataset_dir}/rotate_vis"
...
# }
view_ctrl_info_ls = gen_circular_view_ctrl_info(
    lookat=[0.5, 0.0, 0.0],
    cam_h=0.5,
    cam_r=1.0,
    zoom=0.3,
...
o3d_vis = o3dVisualizer(view_ctrl_info=view_ctrl_info_ls[0], save_path=f'{output_dir}')
o3d_vis.start()
...
# cv2.imwrite(f'{output_dir}/pca_example.png', example_pca_norm)
# cv2.imwrite(f'{output_dir}/example.png', example_img)

is_first = True
o3d_vis.add_triangle_mesh('origin', 'origin')
for i in range(0, 1):
    ...
    # render and save
    for view_i, view_ctrl_info in enumerate(view_ctrl_info_ls):
        pcd_img = o3d_vis.render(['pcd'], save_name=f'raw_pcd_{i}_view_{view_i}', curr_view_ctrl_info=view_ctrl_info)
        cv2.imwrite(f'{output_dir}/vis_{i}_view_{view_i}.png', pcd_img)
        for ch_i in range(3):
            d3fields_img = o3d_vis.render([f'd3fields_{ch_i}'], save_name=f'd3fields_ch_{ch_i}_{i}_view_{view_i}', curr_view_ctrl_info=view_ctrl_info)
            # pca_img = o3d_vis.render(['pca_feats'], save_name=f'pca_feats_{i}')
            partial_d3fields_img = o3d_vis.render([f'partial_d3fields_{ch_i}'], save_name=f'partial_d3fields_ch_{ch_i}_{i}_view_{view_i}', curr_view_ctrl_info=view_ctrl_info)
            blended_img = cv2.addWeighted(pcd_img, 0.4, partial_d3fields_img, 0.6, 0)
            cv2.imwrite(f'{output_dir}/blended_ch_{ch_i}_t_{i}_view_{view_i}.png', blended_img)
    
    os.system(f'ffmpeg -framerate 30 -i {output_dir}/vis_{i}_view_%d.png {output_dir}/vis_{i}.mp4')
    for ch_i in range(3):
        os.system(f'ffmpeg -framerate 30 -i {output_dir}/blended_ch_{ch_i}_t_{i}_view_%d.png {output_dir}/blended_ch_{ch_i}_t_{i}.mp4')
