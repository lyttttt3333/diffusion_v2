...
from d3fields.utils.draw_utils import voxel_downsample
...
def extract_pcd_from_data_dict(data_dict, t, fusion, shape_meta, kin_helper):
    ...
    pcd, pcd_colors = aggr_point_cloud_from_data(color_seq, depth_seq, intri_seq, extri_seq, downsample=False, boundaries=shape_meta['obs']['d3fields']['info']['boundaries'], out_o3d=False, excluded_pts=robot_pcd, exclude_threshold=0.02) # (N, 3), (N, 3)
...
ckpt_path = "/media/yixuan_2T/diffusion_policy/data/outputs/align_shoe/2024.01.30_align_Guang_align_shoe_no_seg_distill_dino_N_4000_pn2.1_r_0.04/checkpoints/latest.ckpt"
data_root = "/media/yixuan_2T/diffusion_policy"
dataset_dir = f"{data_root}/data/outputs/align_shoe/2024.01.30_align_Guang_align_shoe_no_seg_distill_dino_N_4000_pn2.1_r_0.04/eval/method"
...
# # front
# view_ctrl_info = {
#     "front" : [ 0.042116561752864046, 0.92946819388738156, 0.36649020693307249 ],
#     "lookat" : [ 0.3944764601801472, 0.014976268808485257, 0.09045270600805734 ],
#     "up" : [ -0.004479760599095676, -0.36663631745334474, 0.93035355777748541 ],
#     "zoom" : 0.49999999999999978
# }
...
example_pca_norm[~example_mask] = 0.8
...
# for i in range(len(dataset)):
    ...
    vis_shape_meta['obs']['d3fields']['info']['boundaries']['x_lower'] = -0.25
    vis_shape_meta['obs']['d3fields']['info']['boundaries']['x_upper'] = 0.15
    vis_shape_meta['obs']['d3fields']['shape'][1] = 40000
    vis_shape_meta['obs']['d3fields']['info']['distill_obj'] = 'shoe_vis'
    ...
    pcd_vox, pcd_vox_colors = voxel_downsample(pcd, 0.005, pcd_color=pcd_colors)
    ...
    for feat_i in range(distilled_feats.shape[1]):
        d3fields_sim = distilled_feats[:, feat_i]
        ...
        o3d_vis.update_pcd(d3fields_o3d, mesh_name=f'd3fields_{feat_i}')
        o3d_vis.update_pcd(partial_d3fields_o3d, mesh_name=f'partial_d3fields_{feat_i}')
    
    # visualize pca features
    # pcd_vox_in_world = robot_base_pose_in_world @ np.concatenate([pcd_vox, np.ones((pcd_vox.shape[0], 1))], axis=1).transpose() # (4, N)
    # pcd_vox_in_world = pcd_vox_in_world[:3].transpose() # (N, 3)
    # d3fields_vox_bound_mask = (pcd_vox_in_world[:, 0] > vis_boundaries['x_lower']) & (pcd_vox_in_world[:, 0] < vis_boundaries['x_upper']) \
    #                         & (pcd_vox_in_world[:, 1] > vis_boundaries['y_lower']) & (pcd_vox_in_world[:, 1] < vis_boundaries['y_upper']) \
    #                         & (pcd_vox_in_world[:, 2] > vis_boundaries['z_lower']) & (pcd_vox_in_world[:, 2] < vis_boundaries['z_upper'])
    # pcd_vox_tensor = torch.from_numpy(pcd_vox_in_world).to(device='cuda', dtype=torch.float16)
    # vox_feats = fusion.batch_eval(pcd_vox_tensor, return_names=['dino_feats'])['dino_feats'].detach().cpu().numpy()
    # pca_feats = pca.transform(vox_feats) # (N, 3)
    # pca_feats_norm = (pca_feats - pca_norm_min) / (pca_norm_max - pca_norm_min)
    # pca_feats_norm = np.clip(pca_feats_norm, 0, 1)[:,::-1]
    # pca_feats_norm[~d3fields_vox_bound_mask] = 0.8
    # pca_feats_o3d = np2o3d(pcd_vox, color=pca_feats_norm)
    ...
    # visualize action
    pred_act_pos = pred_action_pos[0]
    action_cmap = matplotlib.colormaps.get_cmap('plasma')
    act_len = pred_act_pos.shape[0]
    pred_act_pos_color = action_cmap(np.arange(act_len)/(act_len-1))[:,:3]
    for act_i in range(act_len):
        if is_first:
            o3d_vis.add_triangle_mesh('sphere', f'sphere_{act_i}', color=pred_act_pos_color[act_i], radius=0.005)
        tf = np.eye(4)
        tf[:3, 3] = pred_act_pos[act_i]
        o3d_vis.update_triangle_mesh(f'sphere_{act_i}', tf=tf)
    if is_first:
        is_first = False
    
    # render and save
    action_names_ls = [f'sphere_{act_i}' for act_i in range(act_len)]
    rob_mesh_names_ls = [f'rob_mesh_{mesh_i}' for mesh_i in range(len(rob_mesh))]
    if only_action:
        o3d_vis.render(action_names_ls, save_name=f'3d_action_{i}')
    else:
        o3d_vis.render(action_names_ls, save_name=f'3d_action_{i}')
        pcd_img = o3d_vis.render(['pcd'], save_name=f'raw_pcd_{i}')
        # o3d_vis.render(['pcd'] + action_names_ls + rob_mesh_names_ls, save_name=f'pcd_{i}')
        o3d_vis.render(rob_mesh_names_ls, save_name=f'rob_mesh_{i}')
        pca_img = o3d_vis.render(['pca_feats'], save_name=f'pca_feats_{i}')
        for feat_i in range(distilled_feats.shape[1]):
            d3fields_img = o3d_vis.render([f'd3fields_{feat_i}'], save_name=f'd3fields_feat_{feat_i}_{i}')
            partial_d3fields_img = o3d_vis.render([f'partial_d3fields_{feat_i}'], save_name=f'partial_d3fields_feat_{feat_i}_{i}')
        # blended_img = cv2.addWeighted(pcd_img, 0.6, partial_d3fields_img, 0.4, 0)
        # cv2.imwrite(f'{dataset_dir}/vis/blended_{i}.png', blended_img)
    ...
    for view_idx in range(4):
        ...
        in_bound_mask = (pred_act_pos_in_pix[:, 0] >= 0) & (pred_act_pos_in_pix[:, 0] < img_w) \
                    & (pred_act_pos_in_pix[:, 1] >= 0) & (pred_act_pos_in_pix[:, 1] < img_h)
        ...
        cv2.imwrite(f'{dataset_dir}/vis/action_view_{view_idx}_{i}.png', img)
