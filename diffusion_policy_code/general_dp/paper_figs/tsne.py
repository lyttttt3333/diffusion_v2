...
from diffusion_policy.common.data_utils import load_dict_from_hdf5, d3fields_proc, d3fields_proc_for_vis, d3fields_proc_for_tsne
...
def extract_pcd_from_data_dict(data_dict, t, fusion, shape_meta, auto_wb_path=None):
    ...
    # vis d3fields
    aggr_src_pts_ls, aggr_feats_ls, aggr_raw_feat_ls, _ = d3fields_proc_for_tsne(
        fusion=fusion,
...
cfg_path = "/home/yixuan/bdai/general_dp/general_dp/paper_figs/tsne_cfg.yaml"
...
for i in range(raw_data_dict['observations']['images']['camera_0_color'].shape[0]):
    ...
    for ch_i in range(distilled_feats.shape[1]):
        topk_idx = np.argsort(distilled_feats[:, ch_i])[-k:]
        topk_feats.append(raw_feats[topk_idx])
    topk_feats = np.concatenate(topk_feats, axis=0) # (k*num_ch, 3)
    topk_pcd = np.concatenate([feats_pcd for _ in range(distilled_feats.shape[1])], axis=0)
    
    tsne = TSNE(n_components=2, learning_rate='auto', init='random', perplexity=30)
    # tsne = PCA(n_components=2)
    feat_mean = topk_feats.mean(axis=0)
    feat_std = topk_feats.std(axis=0)
    topk_feats_norm = (topk_feats - feat_mean) / feat_std
    topk_feats_tsne = tsne.fit_transform(topk_feats_norm)
    color_palette = sns.color_palette('husl', n_colors=distilled_feats.shape[1], desat=1)
    labels = ['Shoe Head', 'Shoelace', 'Shoe Tail', 'Mug Handle', 'Mug Ring']
    
    # plot it
    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(8, 8))
    for ch_i in range(distilled_feats.shape[1]):
        plt.scatter(topk_feats_tsne[ch_i*k:(ch_i+1)*k,0], topk_feats_tsne[ch_i*k:(ch_i+1)*k,1], c=[color_palette[ch_i]], label=labels[ch_i])
    plt.legend(prop = font)
    plt.xticks(**font)
    plt.yticks(**font)
    plt.savefig(f'{dataset_dir}/vis/tsne_{i}.png', bbox_inches='tight', dpi=500)
    plt.show()
