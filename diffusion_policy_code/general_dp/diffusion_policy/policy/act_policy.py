...
from diffusion_policy.model.diffusion.build_act import build_ACT_model_and_optimizer
...
class ACTPolicy(BaseImagePolicy):
    def __init__(
        ...
        super().__init__()
        camera_names = [
            k for k in shape_meta["obs"].keys() if shape_meta["obs"][k]["type"] == "rgb"
        ]
        policy_config = {
            "lr": lr,
    ...
    def predict_action(
        ...
        for k in self.shape_meta["obs"].keys():
            if self.shape_meta["obs"][k]["type"] == "spatial":
                ...
                break
        qpos = nobs["ee_pos"][:, 0]
        text_embedding = nobs["embedding"][:, 0]
        if not is_spatial:
            image = [nobs[k][:, 0:1] for k in self.camera_names]
            image = torch.concat(image, dim=1)
            action_pred, _, (_, _) = self.model(
                qpos, image, None, text_embedding
            )  # no action, sample from prior
        else:
            spatial_in = nobs[spatial_key][:, 0:1]
            action_pred, _, (_, _) = self.model(
                qpos, spatial_in, None
            )  # no action, sample from prior
    ...
    def compute_loss(self, batch):
        ...
        if not is_spatial:
            ...
            a_hat, is_pad_hat, (mu, logvar) = self.model(
                qpos, image, None, text_embedding, nactions, is_pad
            )
        else:
            spatial_in = nobs[spatial_key][:, 0:1]
            a_hat, is_pad_hat, (mu, logvar) = self.model(
                qpos, spatial_in, None, nactions, is_pad
            )
