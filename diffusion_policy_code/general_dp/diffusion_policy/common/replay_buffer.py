    ...
    def visualize_data(self, output_dir, vis_distill_feats=False):
        ...
        for l_i in range(loop_num):
            ...
            for i in tqdm(range(self.n_steps)):
                ...
                d3fields = data["d3fields"][0]
                if vis_distill_feats:
                    d3fields_feat = data["d3fields"][0][:, 3 + l_i]
                    d3fields_colors = cmap(d3fields_feat)[:, :3]
                    d3fields_o3d = np2o3d(d3fields, d3fields_colors)
                else:
                    if d3fields.shape[-1] == 4:
                        weight = d3fields[:,3:].reshape(-1,1)
                        color = weight * np.array([[1,0,0]]) + (1-weight) * np.array([[0,0,1]])
                        d3fields_o3d = np2o3d(d3fields[:,:3],color=color)
                    elif d3fields.shape[-1] == 3:
                        d3fields_o3d = np2o3d(d3fields[:,:3])
                    else:
                        raise
                ...
                if vis_cart_action:
                    ...
                    for bot_i in range(num_bot):
                        ...
                        action_rot_mat = pytorch3d.transforms.rotation_6d_to_matrix(
                            torch.from_numpy(action_rot_6d)
                        ).numpy()
                        trans_box_pts = (
                            np.matmul(action_rot_mat, box_pts.T).T + action_pos
                        )
                        box_ls[bot_i].vertices = o3d.utility.Vector3dVector(
                            trans_box_pts
                        )
                ...
                visualizer.capture_screen_image(
                    os.path.join(output_dir, f"{rand_img_id}.png")
                )
                img = cv2.imread(os.path.join(output_dir, f"{rand_img_id}.png"))
                if i == 0:
                    H, W, C = img.shape
                    video = cv2.VideoWriter(
                        os.path.join(output_dir, f"d3fields_{l_i}.mp4"),
