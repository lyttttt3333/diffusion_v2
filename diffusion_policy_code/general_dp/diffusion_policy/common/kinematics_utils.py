...
class KinHelper():
    def __init__(self, robot_name="trossen_vx300s_tactile_thin", headless=True):
        ...
        package_dir = (current_dir / "sapien_env" /
                       "sapien_env" / "assets").resolve()
    ...
    def compute_tool_pcd(self, qpos, tool_name, num_pts=None, pcd_name=None):
        ...
        if tool_name not in self.tool_meshes:
            tool_mesh_path = os.path.join(
                self.package_dir, 'tool', tool_name + '.stl')
        ...
        eef_pose = self.robot_model.get_link_pose(
            eef_link_idx).to_transformation_matrix()
    ...
    def compute_ik_sapien(self, initial_qpos, cartesian):
        ...
        qpos = self.robot_model.compute_inverse_kinematics(
            link_index=self.sapien_eef_idx, pose=pose, initial_qpos=initial_qpos, active_qmask=active_qmask, eps=1e-3, damp=1e-1)
        # verify ik
        fk_pose = self.compute_fk_sapien_links(
            qpos[0], [self.sapien_eef_idx])[0]
        ...
        if pose_diff > 0.01 or rot_diff > 0.01:
            ...
            warnings.warn('ik pose diff or rot diff too large. Return initial qpos.',
...
def test_kin_helper():
    ...
    finger_names = None
    num_pts = None
    init_qpos = np.array([0.851939865243963, -0.229601035617388, 0.563932102437065, -
                         0.098902024821519, 1.148033168114365, 1.016116677288259, 0.0, -0.0])
    end_qpos = np.array([0.788165775078753, -0.243655597686374, 0.573832680057706, -
                        0.075632950397682, 1.260574309772582, 2.000622093036658, 0.0, -0.0])
    ...
    for i in range(100):
        ...
        fk_pose = kin_helper.compute_fk_sapien_links(
            curr_qpos, [kin_helper.sapien_eef_idx])[0]
...
def test_kin_helper_panda():
    ...
    finger_names = None
    num_pts = None
    init_qpos = np.array([-2.21402311, 0.17274992, 2.23800898, -
                         2.27481246, -0.16332519, 2.16096449, 0.90828639, 0.09, 0.09])
    end_qpos = np.array([-2.18224038, 0.26588862, 2.40268749, -
                        2.54840559, -0.2473307, 2.33424677, 1.19656971, 0, 0])
...
def test_fk():
    ...
    for i in range(100):
        curr_qpos = init_qpos + (end_qpos - init_qpos) * i / 100
        fk = kin_helper.compute_fk_sapien_links(
            curr_qpos, [kin_helper.sapien_eef_idx])[0]
        ...
        ik_qpos = kin_helper.compute_ik_sapien(init_ik_qpos, np.array(
            list(fk[:3, 3]) + list(fk_euler)).astype(np.float32))
        re_fk_pos_mat = kin_helper.compute_fk_sapien_links(
            ik_qpos, [kin_helper.sapien_eef_idx])[0]
        re_fk_euler = transforms3d.euler.mat2euler(
            re_fk_pos_mat[:3, :3], axes='sxyz')
        re_fk_pos = re_fk_pos_mat[:3, 3]
        print('re_fk_pos diff:', np.linalg.norm(re_fk_pos - fk[:3, 3]))
        print('re_fk_euler diff:', np.linalg.norm(
            np.array(re_fk_euler) - np.array(fk_euler)))
        ...
        if qpos_diff > 0.01:
            warnings.warn('qpos diff too large',
                          RuntimeWarning, stacklevel=2, )
