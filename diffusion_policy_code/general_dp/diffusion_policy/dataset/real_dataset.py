...
from diffusion_policy.common.rob_mesh_utils import load_mesh, mesh_poses_to_pc
from diffusion_policy.common.data_utils import d3fields_proc, _convert_actions, load_dict_from_hdf5, modify_hdf5_from_dict
...
def _convert_real_to_dp_replay(store, shape_meta, dataset_dir, rotation_transformer,
    ...
    episodes_idx = [int(stem_name.split('_')[-1])
                    for stem_name in episodes_stem_name]
    ...
    for epi_idx in tqdm(episodes_idx, desc=f"Loading episodes"):
        ...
        with h5py.File(dataset_path) as file:
            ...
            for key in lowdim_keys + ['action']:
                ...
                if key == 'action':
                    data_key = 'cartesian_action' if 'key' not in shape_meta[
                        'action'] else shape_meta['action']['key']
                ...
                if key == 'action':
                    ...
                    assert this_data.shape == (
                        episode_length,) + tuple(shape_meta['action']['shape'])
                else:
                    assert this_data.shape == (
                        episode_length,) + tuple(shape_meta['obs'][key]['shape'])
            ...
            for key in rgb_keys:
                ...
                resize_imgs = [cv2.resize(
                    img, (w, h), interpolation=cv2.INTER_AREA) for img in imgs]
            ...
            for key in spatial_keys:
                ...
                distill_dino = shape_meta['obs'][key]['info'][
                    'distill_dino'] if 'distill_dino' in shape_meta['obs'][key]['info'] else False
                ...
                color_seq = np.stack([file['observations']['images'][f'{k}_color'][(
                )] for k in view_keys], axis=1)  # (T, V, H ,W, C)
                depth_seq = np.stack([file['observations']['images'][f'{k}_depth'][(
                )] for k in view_keys], axis=1) / 1000.  # (T, V, H ,W)
                extri_seq = np.stack([file['observations']['images'][f'{k}_extrinsics'][(
                )] for k in view_keys], axis=1)  # (T, V, 4, 4)
                intri_seq = np.stack([file['observations']['images'][f'{k}_intrinsics'][(
                )] for k in view_keys], axis=1)  # (T, V, 3, 3)
                qpos_seq = file['observations']['full_joint_pos'][(
                )] if 'full_joint_pos' in file['observations'] else file['observations']['joint_pos'][()]  # (T, -1)
                if 'robot_base_pose_in_world' in file['observations']:
                    # (T, 4, 4)
                    robot_base_pose_in_world_seq = file['observations']['robot_base_pose_in_world'][(
                    )]
                else:
                    ...
                    robot_base_pose_in_world_seq = np.stack(
                        [robot_base_pose_in_world] * qpos_seq.shape[0], axis=0)
                ...
                if distill_dino:
                    for pts_idx, aggr_src_pts in enumerate(aggr_src_pts_ls):
                        aggr_src_pts_ls[pts_idx] = np.concatenate(
                            [aggr_src_pts, aggr_feats_ls[pts_idx]], axis=-1)

                if key not in spatial_data_dict:
                    spatial_data_dict[key] = list()

                spatial_data_dict[key] = spatial_data_dict[key] + \
                    aggr_src_pts_ls
            ...
            if 'd3fields' in shape_meta['obs'].keys() and use_dino:
                ...
                if is_joint:
                    feats_prefix += '_joint'
                os.system(
                    f'mkdir -p {os.path.join(dataset_dir, f"feats{feats_prefix}")}')
                with h5py.File(os.path.join(dataset_dir, f'feats{feats_prefix}', f'episode_{epi_idx}.hdf5'), 'w') as file:
                    file.create_dataset(
                        'feats', data=feats_per_epi, dtype=np.float32)
...
class RealDataset(BaseImageDataset):
    def __init__(self,
        ...
        if 'd3fields' in shape_meta['obs']:
            ...
            distill_dino = shape_meta['obs']['d3fields']['info'][
                'distill_dino'] if 'distill_dino' in shape_meta['obs']['d3fields']['info'] else False
                ...
                if not os.path.exists(cache_zarr_path):
                    ...
                    for key, attr in shape_meta['obs'].items():
                        if ('type' in attr) and (attr['type'] == 'spatial'):
                            ...
                            fusion = Fusion(num_cam=num_cam,
                                            dtype=self.fusion_dtype)
                            break
        ...
        if vis_input:
            ...
            self.replay_buffer.visualize_data(output_dir=os.path.join(
                dataset_dir, f'replay_buffer_vis_{get_current_YYYY_MM_DD_hh_mm_ss_ms()}'), vis_distill_feats=vis_distill_feats)
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        for key in self.spatial_keys:
            ...
            stat = array_to_stats(
                self.replay_buffer[key][()].reshape(B * N, C))
    ...
    def _sample_to_data(self, sample):
        ...
        for key in self.spatial_keys:
            obs_dict[key] = np.moveaxis(
                sample[key][T_slice], 1, 2).astype(np.float32)
            del sample[key]
...
def test():
    ...
    zarr_path = os.path.expanduser(
        '/media/yixuan_2T/diffusion_policy/data/sapien_env/teleop_data/hang_mug/nescafe_mug_demo_1_closer_views_central_obj')
    shape_meta = {
        'action': {
            'shape': (10,),
            'type': 'low_dim',
...
def update_ee_pose():
    ...
    START_ARM_POSE = [0, -0.96, 1.16, 0, -0.3, 0, 0.02239, -
                      0.02239,  0, -0.96, 1.16, 0, -0.3, 0, 0.02239, -0.02239]
    START_EE_POSE = [2.56418115e-01, -5.50126845e-04,  2.95703636e-01,
                     6.88682872e-04, -3.83402967e-02, -1.18223866e-03,  9.99263804e-01, -0.3]
    ...
    def MASTER_GRIPPER_POSITION_NORMALIZE_FN(x): return (
        x - MASTER_GRIPPER_POSITION_CLOSE) / (MASTER_GRIPPER_POSITION_OPEN - MASTER_GRIPPER_POSITION_CLOSE)

    def PUPPET_GRIPPER_POSITION_NORMALIZE_FN(x): return (
        x - PUPPET_GRIPPER_POSITION_CLOSE) / (PUPPET_GRIPPER_POSITION_OPEN - PUPPET_GRIPPER_POSITION_CLOSE)

    def MASTER_GRIPPER_POSITION_UNNORMALIZE_FN(
        x): return x * (MASTER_GRIPPER_POSITION_OPEN - MASTER_GRIPPER_POSITION_CLOSE) + MASTER_GRIPPER_POSITION_CLOSE

    def PUPPET_GRIPPER_POSITION_UNNORMALIZE_FN(
        x): return x * (PUPPET_GRIPPER_POSITION_OPEN - PUPPET_GRIPPER_POSITION_CLOSE) + PUPPET_GRIPPER_POSITION_CLOSE

    def MASTER2PUPPET_POSITION_FN(x): return PUPPET_GRIPPER_POSITION_UNNORMALIZE_FN(
        MASTER_GRIPPER_POSITION_NORMALIZE_FN(x))

    def MASTER_GRIPPER_JOINT_NORMALIZE_FN(x): return (
        x - MASTER_GRIPPER_JOINT_CLOSE) / (MASTER_GRIPPER_JOINT_OPEN - MASTER_GRIPPER_JOINT_CLOSE)

    def PUPPET_GRIPPER_JOINT_NORMALIZE_FN(x): return (
        x - PUPPET_GRIPPER_JOINT_CLOSE) / (PUPPET_GRIPPER_JOINT_OPEN - PUPPET_GRIPPER_JOINT_CLOSE)

    def MASTER_GRIPPER_JOINT_UNNORMALIZE_FN(
        x): return x * (MASTER_GRIPPER_JOINT_OPEN - MASTER_GRIPPER_JOINT_CLOSE) + MASTER_GRIPPER_JOINT_CLOSE

    def PUPPET_GRIPPER_JOINT_UNNORMALIZE_FN(
        x): return x * (PUPPET_GRIPPER_JOINT_OPEN - PUPPET_GRIPPER_JOINT_CLOSE) + PUPPET_GRIPPER_JOINT_CLOSE

    def MASTER2PUPPET_JOINT_FN(x): return PUPPET_GRIPPER_JOINT_UNNORMALIZE_FN(
        MASTER_GRIPPER_JOINT_NORMALIZE_FN(x))

    def MASTER_GRIPPER_VELOCITY_NORMALIZE_FN(
        x): return x / (MASTER_GRIPPER_POSITION_OPEN - MASTER_GRIPPER_POSITION_CLOSE)

    def PUPPET_GRIPPER_VELOCITY_NORMALIZE_FN(
        x): return x / (PUPPET_GRIPPER_POSITION_OPEN - PUPPET_GRIPPER_POSITION_CLOSE)

    def MASTER_POS2JOINT(x): return MASTER_GRIPPER_POSITION_NORMALIZE_FN(
        x) * (MASTER_GRIPPER_JOINT_OPEN - MASTER_GRIPPER_JOINT_CLOSE) + MASTER_GRIPPER_JOINT_CLOSE

    def MASTER_JOINT2POS(x): return MASTER_GRIPPER_POSITION_UNNORMALIZE_FN(
        (x - MASTER_GRIPPER_JOINT_CLOSE) / (MASTER_GRIPPER_JOINT_OPEN - MASTER_GRIPPER_JOINT_CLOSE))

    def PUPPET_POS2JOINT(x): return PUPPET_GRIPPER_POSITION_NORMALIZE_FN(
        x) * (PUPPET_GRIPPER_JOINT_OPEN - PUPPET_GRIPPER_JOINT_CLOSE) + PUPPET_GRIPPER_JOINT_CLOSE

    def PUPPET_JOINT2POS(x): return PUPPET_GRIPPER_POSITION_UNNORMALIZE_FN(
        (x - PUPPET_GRIPPER_JOINT_CLOSE) / (PUPPET_GRIPPER_JOINT_OPEN - PUPPET_GRIPPER_JOINT_CLOSE))

    MASTER_GRIPPER_JOINT_MID = (
        MASTER_GRIPPER_JOINT_OPEN + MASTER_GRIPPER_JOINT_CLOSE)/2
    ...
    for epi_i in tqdm(range(epi_s, epi_e)):
        ...
        for i in range(old_cartesian_action.shape[0]):
            ...
            puppet_action_old_sec_mat[:3, :3] = transforms3d.euler.euler2mat(
                *puppet_action_old_sec_eef[3:6])
            puppet_action_new_sec_mat = np.linalg.inv(
                robot_base_in_world_seq[i, 0]) @ sec_base_in_world @ puppet_action_old_sec_mat
            ...
            if DEBUG:
                print('original cartesian_action: ',
                      epi_data['cartesian_action'][i])
