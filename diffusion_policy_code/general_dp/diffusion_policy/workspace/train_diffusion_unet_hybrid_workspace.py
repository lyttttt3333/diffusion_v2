...
from diffusion_policy.common.time_utils import get_time_now
    ...
    def run(self):
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                train_losses = list()
                # with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", leave=False, mininterval=cfg.training.tqdm_interval_sec, colour="green", ascii=" 123456789>") as tepoch:
                with tqdm.tqdm(
                    train_dataloader,
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        step_log = {
                            ...
                            "time_hrs": (get_time_now() - start_time).total_seconds()
                            / (60 * 60),
                        }
                ...
                if (self.epoch % cfg.training.val_every) == 0:
                    with torch.no_grad():
                        ...
                        if len(val_losses) > 0:
                            ...
                            step_log["val_action_mse_error"] = np.mean(
                ...
                if (
                    ...
                    self.save_checkpoint(
                        tag=f"epoch={self.epoch:03d}", use_thread=False
                    )
                    time.sleep(1)
