...
@hydra.main(
    version_base=None,
    config_path=str(Path(__file__).parent.parent.joinpath(
        ...
        with h5py.File(dataset_path) as file:
            ...
            for i in range(joint_action.shape[0]):
                ...
                if i == 0:
                    ...
                    waypoint_sphere = o3d.geometry.TriangleMesh.create_sphere(
                        radius=0.03)
                    visualizer.add_geometry(waypoint_sphere)

                curr_pcd.points = pcd_o3d.points
                curr_pcd.colors = pcd_o3d.colors

                waypoint_location = np.array([waypoint[i, 1], -waypoint[i, 0], waypoint[i, 2]]) + np.array([0.5, 0, -0.1])
                waypoint_sphere.translate(waypoint_location, relative=False)
                waypoint_sphere.paint_uniform_color([1 - waypoint[i, -1] * 10, 0, waypoint[i, -1] * 10])

                start_time = time.time()
                while time.time() - start_time < 0.05:
                    visualizer.update_geometry(curr_pcd)
                    visualizer.update_geometry(waypoint_sphere)
