...
data_dir = '/media/yixuan_2T/general_dp/data/sapien_demo/pepsi_demo_60_motion_prim'
...
for i in epi_range:
    ...
    for obs_i, obs in enumerate(obs_ls):
        for t in range(T):
            ...
            cv2.waitKey(1)
            if t == 0:
                vid_ls.append(cv2.VideoWriter(
                    f'{save_dir}/episode_{i}_{obs_keys[obs_i]}.avi', cv2.VideoWriter_fourcc(*'XVID'), 10, (img.shape[1], img.shape[0])))
            vid_ls[obs_i].write(img)
            cv2.imwrite(
                f'{save_dir}/episode_{i}_{obs_keys[obs_i]}_{t}.png', img)
