...
from sapien_env.rl_env.can_env import CanRLEnv
...
class SingleArmPolicy:
    ...
    def grasp_noise(self):
        ...
        return np.array([x_noise, y_noise*2, z_noise])
    ...
    def generate_trajectory(self, env: CanRLEnv, ee_link_pose, mode="straight"):
        init_position = env.manip_obj.get_pose().p
        goal_position = env.box_pos

        if env.manip_obj_name == "cola":
            grasp_h = 0.16
            place_h = 0.19
        ...
        if mode == "straight":
            ...
            self.trajectory = [
                ...
                {
                    "t": 6,
                    "xyz": init_position + np.array([0, 0, grasp_h]) + grasp_offset,
                    "quat": transforms3d.euler.euler2quat(
                        *(ee_init_euler + grasp_rotational_noise)
                    ),
                    "gripper": 0.01,
                ...
                # {
                #     "t": 3,
                #     "xyz": init_position + np.array([0, 0, grasp_h]) +  self.transitional_noise(),
                #     "quat": transforms3d.euler.euler2quat(
                #         *(ee_init_euler + grasp_rotational_noise)
                #     ),
                #     "gripper": 0.01,
