...
class SingleArmPolicy:
    ...
    def interpolate(curr_waypoint, next_waypoint, t):
        ...
        return xyz, quat, gripper, motion_prim
    ...
    def single_trajectory(self, env, ee_link_pose, mode="straight"):
        ...
        xyz, quat, gripper, motion_prim = self.interpolate(
        ...
        return cartisen_action, next_waypoint, motion_prim, quit
    ...
    def generate_trajectory(self, env: StowRLEnv, ee_link_pose, mode="straight"):
        ...
        if env.manip_obj_name == "book_1":
            half_len = 0.1
            half_wid = 0.08
        elif env.manip_obj_name == "book_2":
            raise NotImplementedError
        elif env.manip_obj_name == "flakes_1":
            raise NotImplementedError
        elif env.manip_obj_name == "flakes_2":
            raise NotImplementedError
        else:
            raise NotImplementedError
...
def main_env(count, img_path=None):
    ...
    while True:
        ...
        cartisen_action, _, motion_prim_idx, quit = scripted_policy.single_trajectory(
            env, env.palm_link.get_pose(), mode="straight"
        )
        # transform cartisen_action from robot to world frame
        if quit:
            break
        if last_motion_prim != motion_prim_idx:
            print(f"motion_prim_idx: {motion_prim_idx}")
            last_motion_prim = motion_prim_idx
