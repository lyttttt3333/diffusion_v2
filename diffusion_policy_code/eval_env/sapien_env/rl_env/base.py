...
class BaseRLEnv(BaseSimulationEnv, gym.Env):
    def __init__(
        self, use_gui=True, frame_skip=5, use_visual_obs=False, **renderer_kwargs
    ):
        ...
        self.camera_pose_noise: Dict[
            str, Tuple[Optional[float], sapien.Pose]
    ...
    def arm_sim_step(self, action: np.ndarray):
        ...
        for i in range(self.frame_skip):
            self.robot.set_qf(
                self.robot.compute_passive_force(
                    external=False, coriolis_and_centrifugal=True
                )
            )
            self.scene.step()
        self.current_step += 1
    ...
    def trossen_sim_step(self, action: np.ndarray):
        ...
        target_qpos = np.concatenate(
            [action[0 : self.arm_dof], gripper_action, -gripper_action]
        )
        arm_qvel = action[: self.arm_dof] - self.robot.get_qpos()[: self.arm_dof]
        arm_qvel = np.clip(arm_qvel, -np.pi, np.pi)
        target_qvel = np.zeros_like(target_qpos)
        target_qvel[: self.arm_dof] = arm_qvel
        target_qvel[self.arm_dof :] = (
            target_qpos[self.arm_dof :] - self.robot.get_qpos()[self.arm_dof :]
        )
    ...
    def panda_sim_step(self, action: np.ndarray):
        ...
        target_qpos = np.concatenate(
            [action[: self.arm_dof], gripper_action, gripper_action]
        )
        self.robot.set_drive_target(target_qpos)
        for i in range(self.frame_skip):
            self.robot.set_qf(
    ...
    def setup_visual_obs_config(self, config: Dict[str, Dict]):
        for name, camera_cfg in config.items():
            ...
            if len(banned_modality_set.intersection(set(camera_cfg.keys()))) == len(
                banned_modality_set
            ):
                raise RuntimeError(
                    "Request both point_cloud and depth for same camera is not allowed. "
                    "Point cloud contains all information required by the depth."
                )
        ...
        if "rgb" in modality_set and self.no_rgb:
            raise RuntimeError(
                "Only point cloud, depth, and segmentation are allowed when no_rgb is enabled."
            )
            ...
            if len(self.imagination_infos) > 0:
                ...
                for img_name, points in self.imaginations.items():
                    ...
                    obs_dict[img_name] = gym.spaces.Box(
                        low=-np.inf, high=np.inf, shape=(num_points, 3)
                    )
...
class BimanualBaseRLEnv(BaseSimulationEnv, gym.Env):
    ...
    def setup(self, robot_name, robot_name2):
        ...
        end_joint_name_2 = self.robot2.get_active_joints()[
            self.arm_dof_2 - 1
        ].get_name()
        self.kinematic_model_2 = PartialKinematicModel(
            self.robot2, start_joint_name_2, end_joint_name_2
        )
        self.ee_link_name_2 = self.kinematic_model_2.end_link_name
        self.ee_link_2 = [
            link
            for link in self.robot2.get_links()
            if link.get_name() == self.ee_link_name_2
        ][0]
    ...
    def bimanual_trossen_sim_step(self, action: np.ndarray):
        ...
        for i in range(self.frame_skip):
            ...
            self.robot2.set_qf(
                self.robot2.compute_passive_force(
                    external=False, coriolis_and_centrifugal=False
                )
            )
            self.scene.step()
    ...
    def arm_sim_step_1(self, action: np.ndarray):
        ...
        target_qvel[: self.arm_dof] = arm_qvel
        target_qvel[self.arm_dof :] = (
            action[self.arm_dof :] - self.robot.get_qpos()[self.arm_dof :]
        )
    ...
    def arm_sim_step_2(self, action: np.ndarray):
        ...
        gripper_action = np.clip(gripper_action, 0.01, 0.09)
        target_qpos = np.concatenate(
            [action[0 : self.arm_dof_2], gripper_action, -gripper_action]
        )
        arm_qvel = action[: self.arm_dof] - self.robot.get_qpos()[: self.arm_dof]
        arm_qvel = np.clip(arm_qvel, -np.pi, np.pi)
        target_qvel = np.zeros_like(target_qpos)
