...
class MugCollectRLEnv(MugCollectEnv, BaseRLEnv):
    def __init__(self, use_gui=False, frame_skip=5, robot_name="xarm6", constant_object_state=False,
        ...
        super().__init__(use_gui, frame_skip, object_scale, randomness_scale,
                         friction, use_ray_tracing=False, manip_obj=manip_obj, **renderer_kwargs)
        ...
        self.finger_tip_links = [self.robot.get_links(
        )[robot_link_names.index(name)] for name in finger_tip_names]
    ...
    def get_reward(self, action):
        ...
        obj_box_dist = np.linalg.norm(
            obj_pos[:2] - box_pos[:2])  # ignore z axis
        ...
        reward = (obj_box_dist < obj_box_dist_thresh) and (
            obj_align > obj_align_thresh)
        return float(reward)
    ...
    def reset(self, *, seed: Optional[int] = None, return_info: bool = False, options: Optional[dict] = None):
        ...
        self.object_episode_init_pose = self.object_episode_init_pose * \
            sapien.Pose(random_pos, random_quat)
...
def main_env():
    env = MugCollectRLEnv(use_gui=True, robot_name="panda",
