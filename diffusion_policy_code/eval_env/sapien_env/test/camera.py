...
def main(engine : sapien.Engine, renderer : sapien.SapienRenderer, scene = None):
    if scene is None:
    ...
    urdf_path = 'sapien_env/assets/179/mobility.urdf'
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(num_env)):
        main(engine=engine, renderer=renderer)
    
    # stuck version
    engines = [sapien.Engine() for _ in range(num_env)]
    renderers = [sapien.SapienRenderer(offscreen_only=True) for _ in range(num_env)]
    for i in range(20):
        engines[i].set_renderer(renderers[i])
    scenes = [engine.create_scene() for engine in engines]
    for i in tqdm(range(num_env)):
        main(engine=engines[i], renderer=renderers[i], scene=scenes[i])
