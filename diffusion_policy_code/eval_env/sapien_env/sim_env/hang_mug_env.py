...
class HangMugEnv(BaseSimulationEnv):
    def __init__(
        ...
        **renderer_kwargs,
        ...
        self.select_index = self.np_random.choice(
            range(len(obj_pool)), size=2, replace=false
        )
        manip_obj = [obj_pool[self.select_index[0]], obj_pool[self.select_index[1]]]
    ...
    def get_layout(self, over_write_idx=None):
        if over_write_idx is not None:
            self.mug_idx = over_write_idx[0]
            self.branch_idx = over_write_idx[-1]
        pose_list = []
        ...
        tgt_layout = np.array(
            [
                ...
                [0.0217115, 0.11065497, 0.26605847],
            ]
        )
