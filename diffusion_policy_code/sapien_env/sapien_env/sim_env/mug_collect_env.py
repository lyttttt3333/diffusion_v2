...
# from sapien_env.utils.ycb_object_utils import YCB_SIZE, load_ycb_object
from sapien_env.utils.object_utils import load_open_box, load_obj, load_platform #, load_yx_obj
...
class MugCollectEnv(BaseSimulationEnv):
    def __init__(self, use_gui=True, frame_skip=5, object_scale=1, randomness_scale=1, friction=0.3, seed=None,
                 use_ray_tracing=True, manip_obj="mug_tree", randomness_level='full', **renderer_kwargs):
        super().__init__(use_gui=use_gui, frame_skip=frame_skip,
                         use_ray_tracing=use_ray_tracing, **renderer_kwargs)
        ...
        self.tree = load_platform(
            self.scene, "mug_tree", density=1000, is_static=True)
        
        # Load object
        self.manip_obj_name = manip_obj
        self.manipulated_object = load_obj(
            self.scene, manip_obj, density=1000, is_static=False)
        ...
        if self.randomness_level == 'full':
            if self.box_ls is None:
                ...
                self.box_ls = load_open_box(
                    self.scene, self.renderer, half_l=0.06, half_w=0.06, h=0.02, floor_width=0.005, origin=self.box_pos,)
                self.box_pos = np.array([self.np_random.uniform(-self.wkspc_half_w, self.wkspc_half_w),
                                        self.np_random.uniform(
                                            -self.wkspc_half_l, self.wkspc_half_l),
                                        0.0])
    ...
    def generate_random_init_pose(self):
        ...
        orientation = transforms3d.euler.euler2quat(
            np.pi/2, 0, random_z_rotate)
