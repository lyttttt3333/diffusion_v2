...
from sapien_env.sim_env.constructor import (
...
class BaseSimulationEnv(object):
    ...
    def check_actor_pair_contacts(
        self, actors1: List[sapien.Actor], actor2: sapien.Actor, impulse_threshold=1e-10
    ) -> np.ndarray:
        actor_set1 = set(actors1)
    ...
    def check_actor_contacts_continuous(
        self, actors1: List[sapien.Actor], actor2: sapien.Actor, impulse_threshold=1e-10
    ) -> np.ndarray:
        actor_set1 = set(actors1)
    ...
    def create_table(
        ...
        # Top
        top_pose = sapien.Pose([offset_x, offset_y, -table_half_size[2] + offset_z])
        ...
        if self.renderer and not self.no_rgb:
            ...
            if True:
                ...
                builder.add_box_visual(
                    pose=sapien.Pose(
                        [x + offset_x, y + offset_y, leg_height + offset_z]
                    ),
                    half_size=leg_size,
                    material=table_visual_material,
                    name="leg0",
