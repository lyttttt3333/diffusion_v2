...
from sapien_env.sim_env.stow_env import StowEnv
...
class StowRLEnv(StowEnv, BaseRLEnv):
    def __init__(
        self,
        use_gui=False,
        frame_skip=5,
        ...
        **renderer_kwargs,
        ...
        self.task_level_multimodality=False
    ...
    def get_reward(self, action):
        ...
        manip_obj_pose = self.manip_obj.get_pose()
        
        shelf_pose = self.box.get_pose()

        manip_obj_pos = manip_obj_pose.p
        gt_pos = np.array([0.3026411, 0.12977496, 0.45699388])
        obj_manip_obj_dist = np.linalg.norm(manip_obj_pos[:2] - gt_pos[:2])  # ignore z axis
    ...
    def reset(
        ...
        *,
        ...
        if self.is_xarm:
            ...
            self.robot.set_qpos(qpos)
            self.robot.set_drive_target(qpos)
            init_pos = np.array([0.15, -0.575, 0.0])
...
def main_env():
    ...
    env = StowRLEnv(use_gui=True, use_ray_tracing=True, manip_obj="book_1")
    base_env = env
