...
from sapien_env.sim_env.pack_battery_env import PackBatteryEnv
...
class PackBatteryRLEnv(PackBatteryEnv, BaseRLEnv):
    def __init__(
        self,
        use_gui: bool = False,
        frame_skip: int = 10,
    ...
    def get_reward(self, action: Optional[np.ndarray] = None) -> float:
        ...
        for idx in range(len(self.obj_wait)):
            correct_pos, _ = self.get_obj_done_pose(self.target_idx)
