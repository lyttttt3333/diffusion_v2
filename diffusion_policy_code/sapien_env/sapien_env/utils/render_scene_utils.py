...
def project_points(
    points: np.ndarray, intrinsics: np.ndarray, extrinsics: np.ndarray
) -> np.ndarray:
    assert points.shape[1] == 3
    assert intrinsics.shape == (3, 3)
    assert extrinsics.shape == (3, 4)
    # Add a dimension of 1s to the end of points
    ...
    points_projected = intrinsics @ extrinsics @ points_homogeneous.T
    # Convert back to non-homogeneous coordinates
    points_projected = points_projected / points_projected[-1, :]
    # Only keep the first two coordinates (x and y in the image plane)
    points_2d = points_projected[:2, :].T
    return points_2d
