...
def get_grounding_output_batch_captions(
    model,
    image,
    captions,
    box_thresholds,
...
def grounded_sam(image_path, text_prompt, dino_model, sam_model, box_threshold):
    ...
    for i in range(boxes_filt.size(0)):
        boxes_filt[i] = boxes_filt[i] * torch.Tensor([W, H, W, H]).to(
            device=device, dtype=boxes_filt.dtype
        )
    ...
    save_fn = (
        get_current_YYYY_MM_DD_hh_mm_ss_ms() + str(np.random.randint(9999)) + ".jpg"
    )
    plt.axis("off")
    plt.savefig(save_fn, bbox_inches="tight", dpi=300, pad_inches=0.0)
...
def grounded_sam_batch_queries(
    image_path, text_prompts, dino_model, sam_model, box_thresholds
):
    # cfg
    text_threshold = 0.25
    device = "cuda:0"
    ...
    ending_idx = torch.zeros(len(boxes_filt_list) + 1, dtype=torch.int64).to(
        device=device
    )
    ...
    transformed_boxes = sam_model.transform.apply_boxes_torch(
        boxes_filt_total, image.shape[:2]
    )  # [num_total_boxes, 4]
    ...
    final_masks = torch.zeros(
        (len(boxes_filt_list), H, W), device=device, dtype=torch.float32
    )
    for i in range(len(boxes_filt_list)):
        if ending_idx[i + 1] > ending_idx[i]:
            final_masks[i] = torch.clamp_max(
                masks[ending_idx[i] : ending_idx[i + 1]].sum(dim=0), 1.0
            )[0]
...
def grounded_instance_sam(
    image_path, text_prompt, dino_model, sam_model, box_thresholds
):
    # cfg
    text_threshold = 0.25
    device = "cuda:0"
...
def grounded_instance_sam_new_ver(
    image,
    text_prompts,
    # exclude,
    ...
    # FIXME: Remove the box_thresholds parameter above or below in the future
    BOX_THRESHOLD = 0.25
    TEXT_THRESHOLD = 0.25
    NMS_THRESHOLD = 0.5
    ...
    #     if text_prompts[query_i] in exclude:
    #         detections.xyxy = np.delete(detections.xyxy, query_i, axis=0)
    #         detections.confidence = np.delete(detections.confidence, query_i, axis=0)
    #         detections.class_id = np.delete(detections.class_id, query_i, axis=0)
    ...
    for query_i in detections.class_id.tolist():
        if query_i == None:
            raise ValueError("None class_id detected")
        else:
    ...
    if detections.mask.shape[0] == 0:
        ...
        detections.mask = np.concatenate(
            [np.expand_dims(bg_mask, axis=0), detections.mask], axis=0
        )
    bg_conf = 1.0
    detections.confidence = np.concatenate(
        [np.array([bg_conf]), detections.confidence], axis=0
    )

    return detections.mask, labels, detections.confidence, detections.xyxy

def grounded_instance_given_bbox(
    bbox,
    image,
    sam_model: SamPredictor,
):
    ...
    labels = [f"inst_{i}" for i in range(mask.shape[0])]
    scores = np.ones([mask.shape[0]])
    ...
    bg_conf = 1.0
    scores = np.concatenate([np.array([bg_conf]), scores], axis=0)
    labels = ["background"] + labels

    return mask, labels, scores, bbox

def compute_bbox_for_single_view(camera_config, env_config, img):
    world_pts = load_world_pts(env_config)
    mask = pts_to_mask(world_pts, camera_config, img)
    bbox = mask_to_bbox(mask)
    return bbox

def load_world_pts(env_config):
    pts = np.loadtxt("/home/yitong/diffusion/src_pts/src_battery_model.txt")
    world_pts_list = list()
    for idx in range(env_config.shape[0]):
        world_pts = env_config[idx][:3] + pts
        world_pts_list.append(world_pts)
    return world_pts_list


def pts_to_mask(pts_list, camera_config, img):
    mask_list = list()
    K = camera_config["K"][None, ...]
    Rt = camera_config["Rt"][None, ...]
    device = K.device
    dtype = K.dtype
...
def ground_truth_segment(    
    ...
    inst_num = env_config.shape[0]

    camera_config = dict()
    camera_config["K"] = obs["K"][cam_idx]
    camera_config["Rt"] = obs["pose"][cam_idx]

    xyxy = compute_bbox_for_single_view(camera_config = camera_config,
    ...
    #     bg_mask = ~np.bitwise_or.reduce(mask, axis=0)
    #     mask = np.concatenate(
    #         [np.expand_dims(bg_mask, axis=0), mask], axis=0
    #     )

    labels = ["battery" for _ in range(inst_num)]
    confidence = np.array([1]*inst_num)

    return mask, labels, confidence, xyxy


def grounded_instance_sam_bacth_queries_np(
    image, text_prompts, dino_model, sam_model, box_thresholds, merge_all=False
):
    ...
    for query_i in range(len(boxes_filt_list)):
        labels = labels + [text_prompts[query_i][:-1]] * (
            ending_idx[query_i + 1] - ending_idx[query_i]
        )
    ...
    for i in range(num_masks):
        for j in range(i + 1, num_masks):
            ...
            if IoU > 0.9:
                if (
                    logits_filt_list_cat[i].max().item()
                    > logits_filt_list_cat[j].max().item()
                ):
                    to_rm.append(j)
                else:
                    to_rm.append(i)
...
def grounded_sam_batch_queries_np(
    image, text_prompts, dino_model, sam_model, box_thresholds
):
    # :param image_tensor: [3, H, W]
    assert len(image.shape) == 3
    # cfg
    ...
    if boxes_filt_total.size(0) == 0:
        return torch.zeros(
            (len(boxes_filt_list), H, W), dtype=torch.bool, device=device
        )
    transformed_boxes = sam_model.transform.apply_boxes_torch(
        boxes_filt_total, image.shape[:2]
    )  # [num_total_boxes, 4]
