...
def draw_bbox(img, bbox, color=None):
    if color is not None:
        ...
        color = (0, 255, 0)
    img = cv2.rectangle(
        img, (bbox[0], bbox[1]), (bbox[0] + bbox[2], bbox[1] + bbox[3]), color
    )
    return img
...
def aggr_point_cloud(database, sel_time=None, downsample=True):
    ...
    base_COLMAP_pose = np.concatenate(
        [base_COLMAP_pose, np.array([[0, 0, 0, 1]])], axis=0
    )
    for i in range(start, end, step):
        id = img_ids[i]
        depth = database.get_depth(id, sel_time=sel_time)
        color = database.get_image(id, sel_time=sel_time) / 255.0
        K = database.get_K(id)
...
def color_seg(colors, color_name):
    ...
    colors_hsv = [
        cv2.cvtColor((color * 255).astype(np.uint8), cv2.COLOR_RGB2HSV)
        for color in colors
    ]
    colors_hsv = np.stack(colors_hsv, axis=0)
    colors_hsv = colors_hsv / 255.0
    ...
    if s_cond == "and":
        mask = (
            mask & (colors_hsv[:, :, :, 1] > s_min) & (colors_hsv[:, :, :, 1] < s_max)
        )
    else:
        mask = mask | (
            (colors_hsv[:, :, :, 1] > s_min) & (colors_hsv[:, :, :, 1] < s_max)
        )
    if v_cond == "and":
        mask = (
            mask & (colors_hsv[:, :, :, 2] > v_min) & (colors_hsv[:, :, :, 2] < v_max)
        )
    else:
        mask = mask | (
            (colors_hsv[:, :, :, 2] > v_min) & (colors_hsv[:, :, :, 2] < v_max)
        )
    return mask


def exclude_pts(pcds, excluded_pts,exclude_threshold=0.05):
    pcd_torch = torch.from_numpy(pcds).to(
        device=torch.device("cuda"), dtype=torch.float32
    )  # [N, 3]
    excluded_pts_torch = torch.from_numpy(excluded_pts).to(
        device=torch.device("cuda"), dtype=torch.float32
    ) 
    ...
    idx = idx[0, :, 0]  # [N]
    # [N], True if no point is within exclude_threshold
    min_dist_to_ex_mask = idx == -1
...
def merge_pcd(env_pcd, pcd_to_add, env_num):
    ...
    env_pcd, index = sample_farthest_points(
        pcd_torch[None], K=env_num, random_start_point=False
    )
    env_pcd = env_pcd[0].cpu().numpy()
    full_pcd = np.concatenate([env_pcd, pcd_to_add], axis=0)
    return full_pcd, env_pcd
...
class o3dVisualizer:
    ...
    def render(self, render_names=None, save_name=None, blocking=False):
...
def viewCtrlInterp(view_ctrl_info0, view_ctrl_info1, steps, interp_type="linear"):
    ...
    for i in range(steps):
        ...
        interp_view_ctrl_info.append(
            {
                "front": (
                    np.array(view_ctrl_info0["front"]) * (1 - coeff)
                    + np.array(view_ctrl_info1["front"]) * coeff
                ).tolist(),
                "lookat": (
                    np.array(view_ctrl_info0["lookat"]) * (1 - coeff)
                    + np.array(view_ctrl_info1["lookat"]) * coeff
                ).tolist(),
                "up": (
                    np.array(view_ctrl_info0["up"]) * (1 - coeff)
                    + np.array(view_ctrl_info1["up"]) * coeff
                ).tolist(),
                "zoom": view_ctrl_info0["zoom"] * (1 - coeff)
                + view_ctrl_info1["zoom"] * coeff,
            }
        )
...
def gen_circular_view_ctrl_info(lookat, cam_h, cam_r, zoom, steps):
    ...
    for i in range(steps):
        ...
        cam_pos = np.array(
            [cam_r * np.cos(theta), cam_r * np.sin(theta), cam_h]
        ) + np.array(lookat)
        right = np.array([np.sin(theta), -np.cos(theta), 0])
        front = cam_pos - lookat
        up = np.cross(right, front)
        view_ctrl_info_ls.append(
            {"front": front.tolist(), "lookat": lookat, "up": up.tolist(), "zoom": zoom}
        )
